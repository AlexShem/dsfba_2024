...
- [The **`lubridate`** package](https://lubridate.tidyverse.org/) simplifies working with dates and times in R.
...
- **String Autocomplete**: [`stringr` functions](https://stringr.tidyverse.org/) support autocomplete for easy access to string manipulation tools.
...
- **Combining Strings**: Use [`str_c()`](https://stringr.tidyverse.org/reference/str_c.html) to concatenate strings.
...
- **Handling Missing Values**: Use [`str_replace_na()`](https://stringr.tidyverse.org/reference/str_replace_na.html) to replace `NA` in strings.
...
* See also [`str_to_lower()`](https://stringr.tidyverse.org/reference/case.html), [`str_to_upper()`](https://stringr.tidyverse.org/reference/case.html), or [`str_to_title()`](https://stringr.tidyverse.org/reference/case.html).
...
- **View Pattern Matches**: Use [`str_view()`](https://stringr.tidyverse.org/reference/str_view.html) to visualize pattern matches.
  - `str_view(string, pattern)` shows the pattern matches in the string.
...
```{r}
str_view(stringr::fruit, "(..)\\1")
```

- Imagine as `(..)` capturing the first two characters, and `\\1` referencing the first captured group.
- This example finds words with repeated pairs of letters, like "banana" (`(an)(an)`) or "cucumber" [(`(cu)(cu)`)](https://i.redd.it/gwynh1m2zwab1.jpg).
...
- More advanced patterns with backreferences:
  - Example: Finding words that start and end with the same two letters:
