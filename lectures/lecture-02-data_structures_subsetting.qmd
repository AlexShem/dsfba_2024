...
-   Determine the type and length of a vector with `typeof()` and
    `length()`.
-   Test if a vector is of a given type with `is.*()`:
...
-   The `dim` attribute allow a vector allows it to behave like a
...
-   Value of the double = the number of days since 1970-01-01[^3]:

[^3]: Known as the Unix Epoch.
...
-   The most important S3 vectors built on top of lists:
...
-   Similar to a list, but the length of each component is equal...
...
-   Data frames:
    ...
    -   which lead to the creation of the `tibble`, a modern version.
...
-   Strict rules about what constitutes a valid name.
    -   **Syntactic** names consist of letters[^4], digits, `.` and `_` but can't begin with `_` or a digit.
...
[^4]: what constitutes a letter is determined by your current locale,
    avoid this by sticking to ASCII characters (i.e. A-Z) as much as
    possible.
...
-   Only the first 10 rows + columns that fit on screen.
-   Each column labelled with abbreviated type.
...
-   In `x[y]`, what happens if `x` and `y` are different lengths?
    -   **Recycling rule:** the shorter recycled to the length of the longer.
...
-   Can also subset with an integer matrix.
...
> in car 5; `x[4:6]` is a train of cars 4-6.
>
> --- \@RLangTip,
> <https://x.com/RLangTip/status/268375867468681216>
